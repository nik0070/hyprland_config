...
class Tab {
  ...
  async onUpdated(tabId, {
    ...
    const pageData = await pageDataComponent.create(url);
    ...
    setTimeout(async () => {
      if (pageData.isValidSite) {
        await analysisReporter.addReport({
  ...
  async cleanupTabs() {
    ...
    const pagesData = await pageDataComponent.getAllData();
    const tabData = await this.container.get();
