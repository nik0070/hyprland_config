...
async function getCssFromStorage() {
  const data = await storageService.get('easylistCssValue');
  const {
    extended_rules = {},
    blacklist = {},
    whitelist = {},
  ...
  const parentDomains = host.split('.').map((_, i, arr) => arr.slice(i).join('.')).slice(0, -1);
  for (const list of [extended_rules, cookie_banners_extended_rules]) {
    list[host] = [...new Set([...(list[host] ?? []), ...parentDomains.filter(d => d in list).flatMap(d => list[d])])];
  }
  function getRulesFromList(list) {
    ...
    for (const [pattern, ruleList] of Object.entries(list)) {
      if (pattern === '*') {
        rules.push(...ruleList);
      } else if ([host, ...parentDomains].some(d => new RegExp(`^${pattern.replace(/[-/\\^$+?.()|[\]{}]/g, '\\$&').replace(/\*/g, '.*')}$`, 'i').test(d))) {
        rules.push(...ruleList);
      }
    }
  ...
  if (pageData.enabled) {
    generalRules.push(...getRulesFromList(blacklist));
  ...
  if (pageData.hideCookieBanners) {
    cookieBannersRules.push(...getRulesFromList(cookie_banners_blacklist));
  ...
  return {
    generalRules,
    cookieBannersRules,
    generalExtendedRules: pageData.enabled && extended_rules[host] || [],
    cookieBannersExtendedRules: pageData.hideCookieBanners && cookie_banners_extended_rules[host] || []
  };
}
